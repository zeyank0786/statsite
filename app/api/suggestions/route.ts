...
export async function GET() {
  ...
  try {
    const suggestions = await queryAll(
      `SELECT sg.id, sg.playerId, sg.proposedById, sg.statId, sg.delta, sg.reason,
              sg.status, sg.createdAt, sg.resolvedAt,
 ...
 * ≥1 evidence post — all posted BY the subject and tagged with the stat's
 * category — and the stat must be visible and unlocked for the subject.
 */
export async function POST(request: Request) {
  ...
  try {
    const { subjectPlayerId, statId, delta, reason, evidenceIds } = await request.json();
    ...
    if (!Array.isArray(evidenceIds) || evidenceIds.length === 0) {
      return NextResponse.json(
        { error: 'Attach at least one evidence post — suggestions must be grounded' },
        { status: 400 }
      );
    }
    ...
    // Evidence: must exist, be posted by the subject, and at least one piece
    // must be tagged with the stat's category (keeps suggestions relevant).
    const uniqueEvidenceIds = [...new Set(evidenceIds as string[])];
    ...
    await query(
      `INSERT INTO Suggestion (id, playerId, proposedById, statId, delta, reason, status, createdAt, updatedAt)
       VALUES (?, ?, ?, ?, ?, ?, 'pending', ?, ?)`,
      [id, subjectPlayerId, proposerId, statId, Number(delta), reason.trim(), now, now]
    );
