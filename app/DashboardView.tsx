...
/**
 * The dashboard's presentation layer. All data arrives as props from the
 * server component — nothing here fetches, so the hero score, radar and
 * category bars are in the HTML on first paint.
 *
 * The three self-fetching widgets at the top (ambition takeover, wrapped
 * banner, activity ticker) keep their own polling: each is conditional,
 * frequently renders nothing, and shouldn't block the page on the server.
 */
export default function DashboardView({
  ...
  const { categories, trends, achievements } = data;

  const orderedCategories = orderCategories(categories);
  const overall = computeOverallScore(categories);
  const allStats = orderedCategories.flatMap((cat) =>
    cat.stats.map((s) => ({ ...s, categoryCode: cat.code, categoryLabel: cat.label }))
  );
  const strengths = [...allStats].sort((a, b) => b.value - a.value).slice(0, 5);
  const focusAreas = [...allStats].sort((a, b) => a.value - b.value).slice(0, 5);
  const earnedAchievements = (achievements as AchievementData[]).filter((a) => a.earned);
          ...
          {earnedAchievements.length > 0 ? (
            ...
            <div className="text-center py-6">
              <p className="text-sm mb-3" style={{ color: 'var(--text-secondary)' }}>
                No badges yet — keep grinding. Every stat gain counts.
              </p>
              <Link
                href="/achievements"
...
function StatRow({
  ...
  stat: { label: string; value: number; categoryCode: string };
  accent: string;
}) {
