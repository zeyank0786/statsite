...
import { daysAgo } from './serverStats';
...
export interface PlayerTrends {
  ...
  seriesDates: string[];
  changeCount: number;
  perCategory: { code: string; label: string; avg: number; total: number; net90: number }[];
  net30: number;
  net90: number;
}

/**
 * Reconstructs a player's overall-score timeline from StatHistory and returns
 * per-category momentum for the last 30/90 days.
 *
 * Extracted from the route handler so the server-rendered dashboard can call it
 * directly instead of making an HTTP request to our own API.
 */
export async function computePlayerTrends(id: string): Promise<PlayerTrends> {
